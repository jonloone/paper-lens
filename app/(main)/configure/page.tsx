...
// Logo component - uses actual technology logos from techicons.dev
const Logo = ({ name, className }: { name: string; className?: string }) => {
  // Map the display names to technology keys
  // Extract size from className if provided, default to 'lg'
  let size: 'xs' | 'sm' | 'md' | 'lg' | 'xl' = 'lg';
  if (className?.includes('h-4')) size = 'xs';
  else if (className?.includes('h-5')) size = 'sm';
  else if (className?.includes('h-6')) size = 'md';
  else if (className?.includes('h-8')) size = 'lg';
  else if (className?.includes('h-10')) size = 'xl';
  else if (className?.includes('h-12')) size = 'xl';
  
  // Map display names to technology keys for TechIcon
  ...
  const technology = techMap[name] || name.toLowerCase().replace(/\s+/g, '-');
  return <TechIcon technology={technology} size={size} variant="branded" className={className} />;
};
