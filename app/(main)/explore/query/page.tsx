...
import { launchPlaygroundWithQuery, launchNewQuery } from '@/lib/utils/playground';
...
export default function QueryWorkbenchPage() {
  ...
  const launchPlayground = (query?: SavedQuery) => {
    if (query) {
      const playground = launchPlaygroundWithQuery({
        id: query.id,
        name: query.name,
        sql: query.sql,
      ...
      setPlaygroundWindow(playground.window);
    } else {
      const playground = launchNewQuery({ from: 'workbench' });
      setPlaygroundWindow(playground.window);
    }
  };
