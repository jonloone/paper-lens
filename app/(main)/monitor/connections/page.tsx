...
export default function SourcesPage() {
  ...
  const healthPercentage = totalConnections > 0 ? Math.round((healthyCount / totalConnections) * 100) : 0;
    ...
    <div className="w-full px-8 lg:px-12 xl:px-16 py-6 space-y-4">
      ...
      <div className="bg-background border-b sticky top-14 z-30 -mx-8 lg:-mx-12 xl:-mx-16 px-8 lg:px-12 xl:px-16 pb-3">
        <div className="flex items-center justify-between pt-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Active Source Connections</h1>
              <p className="text-sm text-muted-foreground">Monitor health and performance of your data sources</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-muted/30 rounded-lg px-3 py-1.5">
              <div className={`text-sm font-semibold ${
                ...
                healthPercentage >= 60 ? 'text-yellow-600 dark:text-yellow-400' :
            ...
            <Button
              ...
              disabled={analyzingPriority || (errorCount + warningCount) === 0}
            >
              {analyzingPriority ? (
                <><RefreshCw className="w-4 h-4 mr-1 animate-spin" /> Analyzing...</>
              ) : (
                <><Bot className="w-4 h-4 mr-1" /> Analyze Issues</>
              )}
            </Button>

            <Button
              size="sm"
              onClick={() => router.push('/build/connections')}
            >
      ...
      {errorCount > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <strong>{errorCount} critical issue{errorCount > 1 ? 's' : ''} detected.</strong> {errorCount === 1 ? 'A connection is' : 'Connections are'} experiencing failures affecting {connections.filter(c => c.status === 'error').reduce((sum, c) => sum + c.downstream, 0)} downstream systems.
          </AlertDescription>
        </Alert>
      )}
